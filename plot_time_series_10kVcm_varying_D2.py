...
simulation_result_dirs = [
        r'10kVcmCs=1E+18D1=4E-16_h=1E-12_D2=1E-14_rho=4E-05_SD=1.0_s=5E+01',
        r'10kVcmCs=1E+18D1=4E-16_h=1E-12_D2=1E-15_rho=4E-05_SD=1.0_s=5E+01',
        r'10kVcmCs=1E+18D1=4E-16_h=1E-12_D2=1E-16_rho=4E-05_SD=1.0_s=5E+01',
        r'10kVcmCs=1E+18D1=4E-16_h=1E-12_D2=1E-17_rho=4E-05_SD=1.0_s=5E+01'
    ]

sweep_variable = r'$D_{\mathrm{Si}}$ (cm$^2$/s)'
sweep_variable_units = r'(cm$^2$/s)'
swee_variable_name = r'$D_{\mathrm{Si}}$'
sweep = [1E-14, 1E-15, 1E-16, 1E-17]
sweep_log = True
label_literature = True

results_folder = r'G:\Shared drives\FenningLab2\Projects\PVRD1\Simulations\Sentaurus PID\results\analysis\Cs1E18_E=0.01_variable_D2'
...
if __name__ == '__main__':
    ...
    if label_literature:
        lg2 = ax2.legend(bbox_to_anchor=(1.05, 0.8), loc='upper left', borderaxespad=0.)
    ...
    ax1.plot(
        failure_times['D (cm^2/s)'][idx10_neq_0], failure_times['t 10% loss (h)'][idx10_neq_0],
        ls='--', marker='o', label='10% loss', fillstyle='none'
    )
    
    ax1.plot(
        failure_times['D (cm^2/s)'][idx15_neq_0], failure_times['t 15% loss (h)'][idx15_neq_0],
        ls='--', marker='o', label='15% loss', fillstyle='none'
    )
    
    ax1.plot(
        failure_times['D (cm^2/s)'][idx20_neq_0], failure_times['t 20% loss (h)'][idx20_neq_0],
    ...
    ax1.set_title('$\mathregular{E_{SiN_x}}$ = 10 kV/cm, 85 °C')
    plt.tight_layout()
    fig.savefig(os.path.join(results_folder, 'failure_time_10kVcm_85C.png'), dpi=600)
    
    df_degradation = pd.DataFrame(failure_times)
    df_degradation.to_csv(
        path_or_buf=os.path.join(
            results_folder,
            'failure_time_10kVcm_85C.csv'
        ),
        index=False
    )