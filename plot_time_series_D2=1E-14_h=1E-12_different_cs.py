...
sweep_variable = r'$C_s$ (cm$\mathregular{^{-3}}$)'
sweep_variable_units = r'(cm$\mathregular{^{-3}}$)'
swee_variable_name = r'$C_s$'
sweep = [1E16, 1E18, 1E22]
sweep_log = True
label_literature = False

results_folder = r'G:\Shared drives\FenningLab2\Projects\PVRD1\Simulations\Sentaurus PID\results\analysis\0.5MVcm_D2=1E-14_h=1E-12_variable_Cs'
...
if __name__ == '__main__':
    ...
    ax1.set_title('$\mathregular{h = 10^{-12}\;cm^{2}/s}$, $\mathregular{D_{Si}}$ = $\mathregular{10^{-14}\;cm^2/s}$, 85 °C')
    plt.tight_layout()
    fig.savefig(os.path.join(results_folder, 'failure_time_h=1E-14cm2ps_varying_Cs_h=1E-12cmps_85C.png'), dpi=600)
    
    df_degradation = pd.DataFrame(failure_times)
    df_degradation.to_csv(
        path_or_buf=os.path.join(
            results_folder,
            'failure_times_D2=1E-14_h=1E-12_different_cs.csv'
        ),
        index=False
    )