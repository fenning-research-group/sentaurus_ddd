...
simulation_result_dirs = [
        r'Cs=1E+22D1=4E-16_h=1E-12_D2=1E-18_rho=4E-05_SD=1.0_s=5E+01',
        r'Cs=1E+22D1=4E-16_h=1E-12_D2=1E-16_rho=4E-05_SD=1.0_s=5E+01',
        r'Cs=1E+22D1=4E-16_h=1E-12_D2=1E-14_rho=4E-05_SD=1.0_s=5E+01',
        r'Cs=1E+22D1=4E-16_h=1E-12_D2=1E-12_rho=4E-05_SD=1.0_s=5E+01'
    ]

sweep_variable = r'$D_{\mathrm{Si}}$ (cm$^2$/s)'
sweep_variable_units = r'(cm$\mathregular{^{2}}/s$)'
swee_variable_name = r'$D_{\mathrm{Si}}$'
sweep = [1E-18, 1E-16, 1E-14, 1E-12]
sweep_log = True

results_folder = r'G:\Shared drives\FenningLab2\Projects\PVRD1\Simulations\Sentaurus PID\results\analysis\Cs1E22_h=1E-12_E=0.5_variable_D2'
...
if __name__ == '__main__':
    ...
    for i, lf in enumerate(literature_files):
        ...
        if lf['type'] == 'power':
            ...
            ax1.plot(
                ...
                marker=lf['marker'], zorder=zorder
            )
        else:
            rsh = lit_df['Rsh']
    ...
    # cax1.yaxis.set_major_locator(mpl.ticker.LogLocator(base=10.0, numticks=5))
    # cax1.yaxis.set_minor_locator(mpl.ticker.LogLocator(base=10.0, numticks=50, subs=np.arange(2, 10) * .1))
    ...
    # cax2.yaxis.set_major_locator(mpl.ticker.LogLocator(base=10.0, numticks=5))
    # cax2.yaxis.set_minor_locator(mpl.ticker.LogLocator(base=10.0, numticks=50, subs=np.arange(2, 10) * .1))
    ...
    # cax3.yaxis.set_major_locator(mpl.ticker.LogLocator(base=10.0, numticks=5))
    # cax3.yaxis.set_minor_locator(mpl.ticker.LogLocator(base=10.0, numticks=50, subs=np.arange(2, 10) * .1))
    
    # lg1 = ax1.legend(frameon=False, fontsize=12, loc='lower right')
    # lg2 = ax2.legend(frameon=False, fontsize=12, loc='lower right')
    lg1 = ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
    lg2 = ax2.legend(bbox_to_anchor=(1.05, 0.85), loc='upper left', borderaxespad=0.)
    ...
    idx5_neq_0 = failure_times['D (cm^2/s)'] > 1E-18
    ...
    ax1.plot(
        failure_times['D (cm^2/s)'][idx5_neq_0], failure_times['t 5% loss (h)'][idx5_neq_0],
        ls='--', marker='o', label='5% loss', fillstyle='none'
    )
    
    ax1.plot(
        failure_times['D (cm^2/s)'][idx5_neq_0], failure_times['t 10% loss (h)'][idx5_neq_0],
        ls='--', marker='o', label='10% loss', fillstyle='none'
    )
    
    ax1.plot(
        failure_times['D (cm^2/s)'][idx5_neq_0], failure_times['t 15% loss (h)'][idx5_neq_0],
        ls='--', marker='o', label='15% loss', fillstyle='none'
    )
    
    ax1.plot(
        failure_times['D (cm^2/s)'][idx5_neq_0], failure_times['t 20% loss (h)'][idx5_neq_0],
    ...
    ax1.xaxis.set_minor_locator(mpl.ticker.LogLocator(base=10.0, numticks=40, subs=np.arange(2, 10) * .1))
    
    ax1.set_title('$\mathregular{E_{SiN_x}}$ = 0.5 MV/cm, 85 °C')
    plt.tight_layout()
    fig.savefig(os.path.join(results_folder, 'failure_time_0.5MVcm_85C.png'), dpi=600)
    
    df_degradation = pd.DataFrame(failure_times)
    df_degradation.to_csv(
        path_or_buf=os.path.join(
            results_folder,
            'failure_time_0.5MVcm_85C.csv'
        ),
        index=False
    )