...
simulation_result_dirs = [
        ...
        r'Cs=1E+22D1=4E-16_h=1E-10_D2=1E-14_rho=4E-05_SD=1.0_s=5E+01',
        r'Cs=1E+22D1=4E-16_h=1E-08_D2=1E-14_rho=4E-05_SD=1.0_s=5E+01'
    ]

sweep_variable = r'$h$ (cm/s)'
sweep = [1E-12, 1E-10, 1E-8]
sweep_log = True

results_folder = r'G:\Shared drives\FenningLab2\Projects\PVRD1\Simulations\Sentaurus PID\results\analysis\Cs1E22_D2=1E-14_E=0.5_variable_h'
...
if __name__ == '__main__':
    ...
    for i, fn, sv in zip(range(n_plots), simulation_result_dirs, sweep):
        ...
        ax1.plot(time, pmpp/pmpp[0], color=c_map1(normalize(sv)), zorder=zorder)
        ax2.plot(time, rsh/rsh[0], color=c_map1(normalize(sv)), zorder=zorder)
        ax3.plot(time, ff, color=c_map1(normalize(sv)), zorder=zorder)
        zorder += 1
    ...
    idx5_neq_0 = failure_times[sweep_variable] > 1E-18
    ...
    ax1.plot(
        failure_times[sweep_variable][idx5_neq_0], failure_times['t 10% loss (h)'][idx5_neq_0],
        ls='--', marker='o', label='10% loss', fillstyle='none'
    )
    
    ax1.plot(
        failure_times[sweep_variable][idx5_neq_0], failure_times['t 15% loss (h)'][idx5_neq_0],
        ls='--', marker='o', label='15% loss', fillstyle='none'
    )
    
    ax1.plot(
        failure_times[sweep_variable][idx5_neq_0], failure_times['t 20% loss (h)'][idx5_neq_0],
    ...
    ax1.legend(
        ...
        loc='upper left', ncol=2, mode=None, borderaxespad=0.,
        frameon=False
    )
    ...
    ax1.set_title('$\mathregular{C_s = 10^{22}\;cm^{-3}}$, $\mathregular{D_{Si} = 10^{-14}\;cm^2/s}$, 85 °C')
    plt.tight_layout()
    fig.savefig(os.path.join(results_folder, 'failure_time_Cs_1E22_D2=1E-14_0.5MVcm_85C_varying_h.png'), dpi=600)