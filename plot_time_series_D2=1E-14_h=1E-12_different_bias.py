...
simulation_result_dirs = [
        ...
        r'1kVcmCs=1E+18D1=4E-16_h=1E-12_D2=1E-14_rho=4E-05_SD=1.0_s=5E+01'
    ]

sweep_variable = r'$E$ (MV/cm)'
sweep_variable_units = r'(MV/cm)'
swee_variable_name = r'$E$'
sweep = [0.5, 1E-2, 1E-3]
sweep_log = True

results_folder = r'G:\Shared drives\FenningLab2\Projects\PVRD1\Simulations\Sentaurus PID\results\analysis\Cs1E18_D2=1E-14_h=1E-12_variable_E'
...
if __name__ == '__main__':
    ...
    for i, fn, sv in zip(range(n_plots), simulation_result_dirs, sweep):
        ...
        ax1.plot(time, pmpp/pmpp[0], color=c_map1(normalize(sv)), zorder=zorder, label=sv_txt)
        ax2.plot(time, rsh/rsh[0], color=c_map1(normalize(sv)), zorder=zorder, label=sv_txt)
        # ax3.plot(time, ff, color=c_map1(normalize(sv)))
        zorder += zorder
    ...
    ax1.set_title('$\mathregular{C_s = 10^{18}\;cm^{-3}}$, $\mathregular{D_{Si}}$ = $\mathregular{10^{-14}\;cm^2/s}$, 85 °C')
    plt.tight_layout()
    fig.savefig(os.path.join(results_folder, 'failure_time_1E-14cmps_Cs=1E16cm3_h=1E-12cmps_85C.png'), dpi=600)
    
    df_degradation = pd.DataFrame(failure_times)
    df_degradation.to_csv(
        path_or_buf=os.path.join(
            results_folder,
            'failure_time_1E-14cmps_Cs=1E16cm3_h=1E-12cmps_85C.csv'
        ),
        index=False
    )